...
const costFirstService = require('./config').costFirstService;
const speedFirstService = require('./config').speedFirstService;
...
describe('费用优先 - 分包', () => {
    it('能够根据数量上线分包, 包裹重量包含包裹自身重量',  () => {
        ...
        const distrubution = new Distribution(costFirstService, order);
        const { plan, cost } = distrubution.showMeTheAnswer('money');
        // 总费用
...
describe('速度优先 - 分包', () => {
    it('能够找到速度优先的分包方案, 即使费用高',  () => {
        ...
        const distrubution = new Distribution(speedFirstService, order);
        const { plan, cost } = distrubution.showMeTheAnswer('speed');

        // 总费用
        cost.should.equal(67.2); 
        // 使用一个快递服务
        Object.keys(plan).length.should.equal(1);
        plan.should.have.property('e_1000011');
        // 打两个包
        plan.e_1000011.length.should.equal(2);
        // 第1个包的信息
        plan.e_1000011[0].item.length.should.equal(1); // 第一个包裹里只有一个种类的商品
        plan.e_1000011[0].item[0].name.should.equal(order[0].name);
        plan.e_1000011[0].item[0]._id.should.equal(order[0]._id);
        plan.e_1000011[0].item[0].qty.should.equal(8);
        plan.e_1000011[0].weight.should.equal(10900);
        plan.e_1000011[0].postage.should.equal(38.15);
        // 第2个包的信息
        plan.e_1000011[1].item.length.should.equal(1); // 第一个包裹里只有一个种类的商品
        plan.e_1000011[1].item[0].name.should.equal(order[0].name);
        plan.e_1000011[1].item[0]._id.should.equal(order[0]._id);
        plan.e_1000011[1].item[0].qty.should.equal(6);
        plan.e_1000011[1].weight.should.equal(8300);
        plan.e_1000011[1].postage.should.equal(29.05);
    });
    it('能够在速度相同的时候选择费用最少的分包方案(1个商品)', () => {
        const order = [{
            _id: 'p_1000006',
            name: '爱他美2段',
            weight: 1300,
        ...
        const { plan, cost } = distrubution.showMeTheAnswer('speed');
        // 总费用
        cost.should.equal(25.9);
        Object.keys(plan).length.should.equal(2);
        // 奶粉经济线 - 婴儿 - 成人 1 个包裹
        plan.should.have.property('e_1000002');
        plan.e_1000002.length.should.equal(1);
        plan.e_1000002[0].item.length.should.equal(1); // 第一个包裹里只有一个种类的商品
        plan.e_1000002[0].item[0].name.should.equal(order[0].name);
        plan.e_1000002[0].item[0]._id.should.equal(order[0]._id);
        plan.e_1000002[0].item[0].qty.should.equal(2);
        plan.e_1000002[0].weight.should.equal(3100);
        plan.e_1000002[0].postage.should.equal(10.85);
        // 奶粉BC线 - 3罐 1 个包裹
        plan.should.have.property('e_1000003');
        plan.e_1000003.length.should.equal(1);
        plan.e_1000003[0].item.length.should.equal(1); // 第一个包裹里只有一个种类的商品
        plan.e_1000003[0].item[0].name.should.equal(order[0].name);
        plan.e_1000003[0].item[0]._id.should.equal(order[0]._id);
        plan.e_1000003[0].item[0].qty.should.equal(3);
        plan.e_1000003[0].weight.should.equal(4400);
        plan.e_1000003[0].postage.should.equal(15);
    });
    it('能够在速度相同的时候选择费用最少的分包方案(2个商品)', () => {
        const order = [{
            _id: 'p_1000005',
            name: '爱他美1段',
            weight: 1300,
        ...
        // 奶粉经济线 - 婴儿 1 个包裹 2 个商品
        plan.should.have.property('e_1000003');
        plan.e_1000003.length.should.equal(2);
        // 第1个商品
        plan.e_1000003[0].item[0].name.should.equal(order[0].name);
        plan.e_1000003[0].item[0]._id.should.equal(order[0]._id);
        plan.e_1000003[0].item[0].qty.should.equal(3);
        ...
        plan.e_1000003[0].weight.should.equal(4400);
        // 第2个商品
        plan.e_1000003[1].item[0].name.should.equal(order[1].name);
        plan.e_1000003[1].item[0]._id.should.equal(order[1]._id);
        plan.e_1000003[1].item[0].qty.should.equal(3);
        plan.e_1000003[1].postage.should.equal(15);
        plan.e_1000003[1].weight.should.equal(4400);
    });
});