...
class Distribution {
    ...
    showMeTheAnswer() {
        ...
        return { plan: this.optimalUsePackage, cost: Number(this.optimalCost.toFixed(1)) };
    }
    // 查看整个订单是否满足services
    ...
    takeProductToExpressPackage(productId) {
        ...
        const productInfo = this.productInfoMap[productId]; // 订单中商品的具体描述
        ...
        _.forEach(productRecursiveInfo, (value, index) => {
            ...
            // 限制
            const allSecondTypes = this.servicesInfoMap[serviceInfo._id];
            const secondLevelInfo = _.filter(allSecondTypes.item_types, (item) => item.name === productInfo.type)[0];
            const secondLevelServiceId = secondLevelInfo._id;
            
            let secondLevelRecorder = this.orderExpressPackageInfo[serviceInfo._id].secondInfo[secondLevelServiceId];
            let topLevelRecorder = this.orderExpressPackageInfo[serviceInfo._id].topInfo;
            ...
            while (productCounter !== value) {
                ...
                // 一级限制判断
                // 重量
                if ((topLevelRecorder.weight + productInfo.weight <= serviceInfo.max_weight) || !serviceInfo.max_weight) {
                    allCheckFlag--;
                }
                // 价值
                if ((topLevelRecorder.value + productInfo.value <= serviceInfo.max_value) || !serviceInfo.max_value || !productInfo.value) {
                    allCheckFlag--;
                }
                // 数量
                if ((topLevelRecorder.qty + 1 <= serviceInfo.max_qty) || !serviceInfo.max_qty) {
                    allCheckFlag--;
                }
                // 二级限制判断
                if ((secondLevelRecorder.weight + productInfo.weight) <= secondLevelInfo.max_weight || !secondLevelInfo.max_weight) {
                    allCheckFlag--;
                }
                if ((secondLevelRecorder.value + productInfo.value) <= secondLevelInfo.max_value || !secondLevelInfo.max_value || !productInfo.value) {
                    allCheckFlag--;
                }
                if ((secondLevelRecorder.qty + 1 <= secondLevelInfo.max_qty) || !secondLevelInfo.max_qty) {
                    allCheckFlag--;
                }
                ...
                if (allCheckFlag !== 0) {
                    ...
                    this.initOrderExpressPackageByServiceId(serviceInfo._id);
                    secondLevelRecorder = this.orderExpressPackageInfo[serviceInfo._id].secondInfo[secondLevelServiceId];
                    topLevelRecorder = this.orderExpressPackageInfo[serviceInfo._id].topInfo;
                }
    ...
    calculationCost() {
        ...
        for(let expressId in this.usePackageCounter) {
            ...
            _.forEach(packages, (packageInfo) => {
                ...
                let packageWeight = 500;
                let tmp = [];
                for(let productId in packageInfo) {
                    min_qty += packageInfo[productId];
                    packageWeight += this.productInfoMap[productId].weight * packageInfo[productId];
                    tmp.push({
                        _id: productId,
                ...
                let packageRate = null;
                // 如果包裹不到min_weight那么最终以一个rate结算
                // 包裹自重500g
                if (packageWeight < expressInfo.min_weight && expressInfo.min_weight) {
                    packageRate = Number(expressInfo.rate);
                } else {
                    // 如果满足最小重量限制，且包裹存在固定价格
                    packageRate = minLimitValid && expressInfo.fixed_rate ? expressInfo.fixed_rate : Number((expressInfo.rate * packageWeight / 1000).toFixed(2));
                } 
                allCost += packageRate;
        ...
        let optimalUsePackageNumber = 0;
        _.forEach(this.optimalUsePackage, (packages) => optimalUsePackageNumber += packages.length);
        let tmpUseServiceNumber = 0;
        _.forEach(tmpPackageInfo, (packages) => tmpUseServiceNumber += packages.length);
        
        if (allCost === this.optimalCost && tmpUseServiceNumber < optimalUsePackageNumber) {
            this.optimalUsePackage = tmpPackageInfo;
            this.optimalCost = allCost;
        }
    }
