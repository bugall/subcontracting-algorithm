    ...
    calculationCost() {
        ...
        for(let expressId in this.usePackageCounter) {
            ...
            _.forEach(packages, (packageInfo) => {
                ...
                // 包裹自重500g
                if (packageWeight + 500 < expressInfo.min_weight && expressInfo.min_weight) {
                    packageRate = Number(expressInfo.rate);
                } else {
                    // 如果满足最小重量限制，且包裹存在固定价格
                    packageRate = minLimitValid && expressInfo.fixed_rate ? expressInfo.fixed_rate : Number((expressInfo.rate * (packageWeight + 500) / 1000).toFixed(2));
                } 
                allCost += packageRate;
