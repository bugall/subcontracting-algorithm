...
class Distribution {
    ...
    getNotFullBagAndCanPutOneMore(serviceId, productId) {
        
    }
    // 向serviceId 的 第 number 个包里放productId这个商品
    checkCanPutOneMoreProductInBag(serviceId, BagNumber, productId) {
        // 找到快递服务的对应的具体信息
        ...
        const secondLevelInfo = _.filter(allSecondTypes.item_types, (item) => item.name === this.productInfoMap[productId].type)[0];
        ...
        let allCheckFlag = 6;
        topLevelRecorder.weight + productInfo.weight <= serviceInfo.max_weight || !serviceInfo.max_weight ? allCheckFlag-- : '';
        topLevelRecorder.value + productInfo.value <= serviceInfo.max_value || !serviceInfo.max_value || !productInfo.value ? allCheckFlag-- : '';
        topLevelRecorder.qty + 1 <= serviceInfo.max_qty || !serviceInfo.max_qty ? allCheckFlag-- : '';
        secondLevelRecorder.weight + productInfo.weight <= secondLevelInfo.max_weight || !secondLevelInfo.max_weight ? allCheckFlag-- : '';
        secondLevelRecorder.value + productInfo.value <= secondLevelInfo.max_value || !secondLevelInfo.max_value || !productInfo.value ? allCheckFlag-- : '';
        secondLevelRecorder.qty + 1 <= secondLevelInfo.max_qty || !secondLevelInfo.max_qty ? allCheckFlag-- : '';
        return allCheckFlag === 0 ? true : false;
    }
    ...
    putProductInBag(productId) {
        ...
        _.forEach(productRecursiveInfo, (value, index) => {
            ...
            this.checkUserNewBagOrOld(serviceId);
            // 一个一个的装包，直到商品装完
            while (productCounter !== value) {
                if (allCheckFlag !== 0) {
                    if (_.isEmpty(this.useBagCounter[serviceInfo._id])) {
                        this.useBagCounter[serviceInfo._id] = [{[productInfo._id]: singleBagProductCounter}];
                    } else {
                        if (useNewBag || singleBagProductCounter === 0) {
                            this.useBagCounter[serviceInfo._id].push({[productInfo._id]: singleBagProductCounter});
                        } else {
                            const leng = this.useBagCounter[serviceInfo._id].length - 1;
                            this.useBagCounter[serviceInfo._id][leng][productInfo._id] = singleBagProductCounter;
                        }
                    }
                    useNewBag = true;
    ...
    calculationCost() {
        console.log(this.orderExpressBagInfo);
        const tmpBagInfo = {};
        let allCost = 0;
