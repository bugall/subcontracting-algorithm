...
class Distribution {
    ...
    calculationCost() {
        ...
        for(let expressId in this.useBagCounter) {
            ...
            allCost += bagCost;
            if (!minLimitValid) validFlag = false;
        }
        if (allCost < this.optimalCost && validFlag) {
        ...
        let tmp = 0;
        _.forEach(Object.keys(this.optimalUseBag), (expressId) => {
        ...
        // 速度相同取费用最小的
        if (this.optimalSpeedFirstCounter === tmp && this.optimalSpeedFirstCost > allCost) {
            this.optimalSpeedFirst = bagInfo;
            this.optimalSpeedFirstCounter = tmp;
            this.optimalSpeedFirstCost = allCost;
        }
